...
export function CoffeeCard({ coffee }: CoffeeCardProps) {
  return (
    <Link href={{ pathname: "/coffee/[id]", params: { id: coffee.id } }} asChild>
      <Pressable>
        ...
        <GlassCard className="rounded-[32px]" contentClassName="p-3">
          <View className="flex-row gap-3">
            <View className="overflow-hidden rounded-[28px] bg-latte">
              {coffee.photoUri ? (
                <Image source={{ uri: coffee.photoUri }} className="h-28 w-28" />
              ) : (
            ...
            <View className="flex-1 gap-2">
              <View className="flex-row justify-between gap-3">
                ...
                {coffee.isFavorite ? <Heart size={18} color="#C96B38" fill="#C96B38" /> : null}
              </View>
              <View className="flex-row flex-wrap gap-2">
                <View className="flex-row items-center gap-1 rounded-full bg-white/70 px-2 py-1 dark:bg-white/10">
                  ...
                  <BrewText className="text-sm font-semibold">{coffee.rating}/10</BrewText>
                </View>
                <View className="flex-row items-center gap-1 rounded-full bg-white/70 px-2 py-1 dark:bg-white/10">
                  <MapPin size={14} color="#7C9A78" />
                  <BrewText className="text-sm">{coffee.source}</BrewText>
                </View>
                <BrewText className="rounded-full bg-cocoa px-2 py-1 text-sm font-bold text-crema">{formatCurrency(coffee.price)}</BrewText>
              </View>
