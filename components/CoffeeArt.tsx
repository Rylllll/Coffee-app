...
export function CoffeeArt({
  ...
  const cupSize =
    size === "lg" ? "h-72 w-56" : size === "sm" ? "h-28 w-24" : "h-48 w-40";
  const creamSize =
    size === "lg" ? "h-28 w-52" : size === "sm" ? "h-12 w-24" : "h-20 w-36";
  ...
  return (
    <View className={cn("items-center justify-center", className)}>
      ...
      <View
        className={cn(
          "relative overflow-hidden rounded-b-[42px] rounded-t-[72px] border border-white/70 bg-white/45",
          cupSize,
        )}
      >
      ...
      <View
        className={cn(
          "absolute top-1 rounded-[48px] bg-crema shadow-sm",
          creamSize,
        )}
      >
