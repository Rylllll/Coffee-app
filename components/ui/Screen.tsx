...
export function Screen({ children, scroll = true, className, contentClassName }: ScreenProps) {
  ...
  const colors = isDark ? (["#170805", "#2B120C", "#120705"] as const) : (["#FFF9EF", "#F4E5D7", "#DFC8B4"] as const);
  const content = <View className={cn("w-full max-w-[460px] self-center gap-5 px-5 pb-28 pt-2", contentClassName)}>{children}</View>;

  return (
    <SafeAreaView className={cn("flex-1 bg-mist dark:bg-espresso", isDark && "dark", className)}>
