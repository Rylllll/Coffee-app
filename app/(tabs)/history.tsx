...
export default function HistoryScreen() {
  ...
  const visible = useMemo(() => {
    if (filter === "Cafe")
      return coffees.filter((entry) => entry.source === "Cafe");
    if (filter === "Homemade")
      return coffees.filter((entry) => entry.source === "Homemade");
    if (filter === "Favorites")
      return coffees.filter((entry) => entry.isFavorite);
    return coffees;
  }, [coffees, filter]);
  ...
  return (
    <Screen>
      ...
      <SectionHeader
        eyebrow={`${visible.length} memories`}
        title="Your logged coffees"
      />
      <View className="gap-3">
        {visible.length > 0 ? (
          ...
          <EmptyState
            title="No coffees here yet"
            body="Try another filter or log a new coffee memory."
          />
        )}
      </View>
    </Screen>
