...
export const estimateCaffeine = (type: CoffeeType, override?: number) =>
  override ?? caffeineByType[type] ?? 120;
...
export const getCaffeineSummary = (
  ...
  const weekMg = entries
    .filter((entry) => isInLastDays(entry.drankAt, 7))
    .reduce((sum, entry) => sum + entry.caffeineMg, 0);
  const monthMg = entries
    .filter((entry) => isInLastDays(entry.drankAt, 30))
    .reduce((sum, entry) => sum + entry.caffeineMg, 0);
  const latestLateCoffee = entries.find(
    (entry) => new Date(entry.drankAt).getHours() >= 17,
  );
...
export const getCoffeeDNA = (entries: CoffeeEntry[]): CoffeeDNA => {
  if (entries.length === 0) {
    return {
      ...
      insightLines: [
        "Log your first coffee to start building your Coffee DNA.",
      ],
    };
  }

  const favoriteType = topKey(
    countBy(entries, (entry) => entry.type),
  ) as CoffeeDNA["favoriteType"];
  const favoriteRoast = topKey(
    countBy(
      entries.filter((entry) => entry.roastLevel),
      (entry) => entry.roastLevel ?? "Unknown",
    ),
  ) as CoffeeDNA["favoriteRoast"];
  const averageSpend =
    entries.reduce((sum, entry) => sum + entry.price, 0) / entries.length;
  const milkBased = entries.filter((entry) =>
    ["Latte", "Cappuccino", "Flat White", "Mocha", "Matcha Latte"].includes(
      entry.type,
    ),
  ).length;
  ...
  const persona =
    weekendCount >= Math.ceil(entries.length / 3)
      ? "Weekend Explorer"
      : "Ritual Optimizer";
  const spendingBehavior =
    averageSpend >= 6
      ? "Premium cafe spender"
      : "Balanced home-and-cafe spender";
  const preferredTimeWindow =
    morningCount >= Math.ceil(entries.length / 3)
      ? "8 AM to 10 AM"
      : "Flexible afternoon window";
  ...
  return {
    ...
    insightLines: [
      `You are a ${persona}.`,
      milkBased >= entries.length / 2
        ? "You prefer sweet milk-based drinks."
        : "You lean toward cleaner black coffee profiles.",
...
export const getWrappedReport = (entries: CoffeeEntry[]): WrappedReport => {
  const sorted = [...entries].sort(
    (a, b) => new Date(a.drankAt).getTime() - new Date(b.drankAt).getTime(),
  );
  const dayCounts = countBy(entries, (entry) =>
    new Date(entry.drankAt).toLocaleDateString("en-US", { weekday: "long" }),
  );
  ...
  sorted.forEach((entry) => {
    ...
    if (!lastDate || sameCalendarDay(current, lastDate)) {
      ...
      const diff = Math.round(
        (current.setHours(0, 0, 0, 0) - lastDate.setHours(0, 0, 0, 0)) / dayMs,
      );
...
export const getProductivityInsight = (
  entries: CoffeeEntry[],
  ...
  const linked = sessions
    ...
    .filter((item): item is { session: WorkSession; coffee: CoffeeEntry } =>
      Boolean(item.coffee),
    );

  const best = linked.sort(
    (a, b) => b.session.productivityScore - a.session.productivityScore,
  )[0];
  if (!best) {
  ...
  const when =
    hour < 10 ? "before 10 AM" : hour < 14 ? "near lunch" : "later in the day";
  return `Your most productive days happen after drinking a ${best.coffee.type} ${when}.`;
};

export const getPassportCountries = (
  entries: CoffeeEntry[],
  beans: BeanCollectionItem[],
) => {
  const countries = new Set<string>();
  entries.forEach(
    (entry) => entry.originCountry && countries.add(entry.originCountry),
  );
...
export const getBeanExpiryLabel = (bean: BeanCollectionItem) => {
  const days = Math.ceil(
    (new Date(bean.expiresAt).getTime() - Date.now()) / dayMs,
  );
